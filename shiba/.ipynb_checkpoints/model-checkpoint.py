...
class Shiba(torch.nn.Module):
    ...
    # def _repeat_molecules(self, molecules: torch.Tensor, char_seq_length: int):
    #     repeated_molecules = molecules.unsqueeze(1).repeat(1, self.config.downsampling_rate, 1)
    #     repeated_molecules = repeated_molecules.view(-1, molecules.size(1), molecules.size(2))

    #     remainder_length = char_seq_length % self.config.downsampling_rate

    #     # Repeat the last molecule to reach a multiple of `self.config.downsampling_rate`
    #     last_molecule = molecules[:, -1:, :]
    #     last_molecule_repeated = last_molecule.repeat(1, remainder_length, 1)
